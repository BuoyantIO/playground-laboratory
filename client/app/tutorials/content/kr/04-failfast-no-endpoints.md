...
목적지 Service의 준비된 엔드포인트가 0개일 때, 아웃바운드 로드 밸런서는 **failfast** 상태로 진입하고 요청은 버퍼링 없이 즉시 `504`로 실패합니다.

현장에서 가장 자주 오진단되는 Linkerd 오류입니다. 타임아웃처럼 보이지만, 프록시는 서버가 아닌 엔드포인트의 *존재* 자체를 기다리고 있습니다.

> 이 오류는 클라이언트와 서버 프록시 사이의 TLS 연결이
> `config.linkerd.io/proxy-outbound-connect-timeout` 시간 안에
> 수립되지 못하는 네트워크 문제로도 발생할 수 있습니다(기본 1000 ms).

## 설치

[00-setup.md](00-setup.md)을 따라 클러스터, Linkerd Enterprise, 플레이그라운드 앱을 준비합니다. 진행 전 UI에 녹색 `200`과 `mTLS` 배지가 표시되어야 합니다.

## 증상

- 클라이언트 UI: 모든 폴링이 ~3초 내에 빨간 `504`로 바뀝니다.
- 지연 시간이 ~3000 ms에 고정됩니다. 프록시가 서버를 기다리는 것이 아니라 단락(short-circuit)시키기 때문입니다.
- `mTLS` 배지는 빨강/공백으로 유지됩니다(래핑할 응답이 없음). 토폴로지 배너가 빨개지는 것이 핵심 신호로, 프록시가 호출을 실패시키고 있다는 뜻입니다.

## 재현

primary와 canary 디플로이먼트를 **모두** 0으로 스케일합니다. canary를 남기면 kube-proxy가 그쪽으로 라우팅해 요청이 성공하게 됩니다.
...
메시 클라이언트에서 직접 curl로 응답 헤더를 확인할 수 있습니다.
...
기본 아웃바운드 HTTP failfast 타임아웃은 `DEFAULT_OUTBOUND_HTTP_FAILFAST_TIMEOUT` 상수에 의해 **3초**로 설정됩니다. 엔드포인트가 0개인 채 3초가 경과하면 큐가 failfast로 진입하고 모든 요청에 `FailFastError`를 발생시키며, rescue 핸들러가 이를 `gateway_timeout`을 통해 합성 504로 변환합니다.
...
프록시는 약 1초 안에 새 엔드포인트를 감지하고, 다음 요청에서 밸런서가 failfast를 빠져나옵니다.
