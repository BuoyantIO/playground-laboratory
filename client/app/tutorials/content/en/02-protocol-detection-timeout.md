...
~10 seconds, the proxy classifies the connection as **opaque** and forwards
it as raw TCP. The connection is **not** closed, only delayed by the
detection timeout.

This "server-speaks-first" / "neither side speaks" failure mode is a common
gotcha for protocols like SSH, IRC, or any custom port not on the
opaque-ports list. (MySQL, SMTP, Postgres, Redis, Memcached, and a few
others are already in Linkerd's `default-opaque-ports` list and are
unaffected in a default install.)

To observe the timeout, open an empty-payload TCP connection from a meshed
pod and watch the proxy log.
...
- The server-side proxy receives the connection via the transport header
  but skips detection because the outbound proxy sent no session-protocol
  hint, so there is no matching inbound timeout log.

This is invisible in the SMA UI: the Next.js client sends an HTTP preamble
immediately, which the proxy detects in microseconds. The timeout is
demonstrated via `nc` from inside a meshed pod.
...
The 15 s elapsed because `timeout 15` killed `nc`, not because the proxy
waited 15 s. The proxy's own timeout fired at the 10 s mark.
...
The `Continuing after timeout` line is the key signal: the proxy switched
to the opaque (raw-TCP) handler without closing the connection.
...
You will *not* see a `ReadTimeout` line on the inbound side. The outbound
proxy sent a transport header without a session-protocol hint, so the
inbound side routes directly to the opaque/forward stack and skips
byte-level HTTP detection.

You may see `Detected result=Ok(Http(HTTP/1)) elapsed=Xµs` lines from the
SMA traffic generator (each completing in microseconds because real HTTP
bytes arrive immediately). These are unrelated to the stalled `nc`
connection.
...
(`proxy.detect_protocol_timeout`). When the proxy accepts an outbound TCP
stream on a port whose outbound policy is `Detect`:
...
3. **If no bytes arrive within 10 s**: detection returns
   ...
   timeout: 10s` and falls through to the opaque stack, forwarding the
   connection as raw TCP for the rest of its life.

The connection is **not** torn down. The peer (here `nc`) sees no error;
the only externally visible effect is that the first 10 s produce no
forwarded traffic.

Two real-world failure patterns:

- **Server-speaks-first protocols** (SSH, IRC, custom TCP). The server
  wants to send a banner but cannot because the proxy is waiting for the
  client to speak first. Both sides wait. After 10 s the proxy falls back
  to opaque and the banner gets through, but every new connection pays
  the 10 s cost.
- **Slow first byte on an HTTP server**: e.g. a thread-pool-starved server
  that won't write the request line for > 10 s. The proxy gives up on HTTP,
  treats the stream as opaque, and forwards whatever eventually arrives, but
  per-request route features (timeouts, retries, metrics) are lost for that
  connection.

The fix is the same in both cases: mark the port as opaque so the proxy
skips detection and forwards bytes immediately. For HTTP services that are
merely slow at startup, the right fix is application-side, not mesh config.
...
If the destination is a server-speaks-first protocol, mark its port opaque
at workload-pod level (controls the inbound proxy's local policy):
