...
AuthorizationPolicy, and similar.
...
- The `l5d-client-id` request header the inbound proxy normally injects on
  meshed HTTP calls. mTLS still happens at the TCP layer (`tls="true"` on
  every `tcp_open_total` row), but apps that surface mTLS identity from the
  HTTP header will report it as missing.

In the playground specifically, this last point is observable in the UI:
the server reads `l5d-client-id` from the inbound request and forwards it
as `X-Mesh-Client-Id` on the response. Under opaque-ports, that header is
empty, so the UI's mTLS badge flips to **plain**. In apps without that
indirection, the badge would stay green and the failure would be silent,
which is the variant most teams hit in production.
...
- Outbound `request_total{authority="playground-server-http..."}` stops
  incrementing on the client proxy, even as polls continue.
- Previously-working HTTPRoute timeouts/retries silently no-op.

## Where the annotation lives

`config.linkerd.io/opaque-ports` can be set in three places, and each one
affects a *different* side of the connection. Understanding which is which
is the whole point of this runbook.
...
performance" and silently strips outbound HTTP visibility from every caller,
while server-side metrics keep working.

The fix is always the same: decide which side needs to be opaque (or
both), and annotate accordingly.

## Recreate

### 1. Opaque port on the Service

Annotating the Service tells **every outbound caller** to skip HTTP
detection for this Service+port. The destination controller picks up the
annotation and publishes `protocol.Kind: Opaque` in the outbound policy;
outbound proxies then route bytes through the opaque path. The server's
inbound proxy is unaffected, it still parses HTTP and still injects
`l5d-client-id`.

Before changing anything, let's inspect the policy via:
...
The `Detect` block means that the outbound proxy will detect the protocol.
...
**Verify on the control plane** the announced protocol should have
flipped from `Detect` to `Opaque`:
...
**Verify on the data plane**, no HTTP request counter should exist for
this authority on the client's outbound proxy:
...
The cleanest possible signature is that **no `request_total` rows for this
authority exist at all**. The proxy didn't just stop incrementing the HTTP
counter, it never instantiated the HTTP balancer for this destination, so
no Prometheus series was ever created. Only `tcp_open_total` remains, and
it stays at 1 thanks to HTTP/1 keepalive on the application side.
...
**Bonus check: HTTPRoute-layer policy is bypassed too.** Apply a 1 ms
timeout and add 2 s of server-side latency; the timeout would normally
fire but won't, because the outbound proxy isn't running the HTTP stack
that HTTPRoute attaches to:
...
A 1 ms timeout against a 2 s server should fire. Under opaque, it doesn't, 
the request still takes 2 s and succeeds.

Before moving on to case 2, undo the changes so the two cases stay
independent:
...
Annotating the pod template flips only the **server's own inbound proxy**.
The proxy-injector reads the annotation at admission time and sets
`LINKERD2_PROXY_INBOUND_PORTS_DISABLE_PROTOCOL_DETECTION=8080` on the
sidecar. The destination controller is uninvolved; outbound callers never
learn the port is opaque.
...
**Verify the control plane is still announcing `Detect`** because the
Service is not annotated, outbound callers should be unaffected:
...
**Verify on the data plane** client side still parsing HTTP, server side
no longer:
...
The client side keeps climbing; the server side stops at whatever value it
held when the annotation took effect.

**What you see in the UI**:

- Status code stays `200`, latency normal.
- mTLS badge **flips to `plain`** the server's inbound proxy no longer
  injects `l5d-client-id`, so `X-Mesh-Client-Id` on the response is empty.
  This is a playground-specific signal (the app surfaces the header
  directly); in a typical workload the equivalent would be silent.
- Any inbound HTTPRoute or AuthorizationPolicy attached to the server's HTTP layer
  silently no-ops on this port. Outbound HTTPRoutes (the common case, with
  `parentRefs: Service`) **still apply** here, because the Service is not
  annotated and the client's outbound proxy is still in HTTP mode.
...
When opaque ports *are* the right answer: server-speaks-first protocols
(MySQL `3306`, Postgres `5432`, Redis `6379`, SMTP `25`). HTTP/1, HTTP/2,
gRPC ports should **never** be marked opaque.
