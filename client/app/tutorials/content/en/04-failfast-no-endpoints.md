...
The "scaled to zero" scenario. When the destination Service has zero ready
endpoints, the outbound load balancer enters **failfast** and returns `504`
immediately instead of buffering indefinitely.

This is the most-misdiagnosed Linkerd error. It looks like a timeout, but the
proxy is waiting for an endpoint to *exist*, not for the server to respond.

> This error can also be caused by networking issues that prevent TLS
> connections between the client and server proxies within the
> `config.linkerd.io/proxy-outbound-connect-timeout` window (default 1000 ms).

## Setup

Follow [00-setup.md](00-setup.md) for a fresh cluster, Linkerd Enterprise, and
the playground app. Confirm green `200`s with `mTLS` badges in the UI before
proceeding.

## Symptom

- Client UI: every poll turns red `504` within ~3 s.
- Latency is a flat ~3000 ms; the proxy is short-circuiting, not waiting on
  the server.
- The "mTLS" badge stays red/empty (no response to wrap); the topology banner
  turns red.

## Recreate

Scale **both** the primary and canary deployments to zero; leaving the canary
running would let kube-proxy keep routing to it:
...
A curl from inside the meshed client confirms the response headers:
...
Endpoint membership from the destination controller:
...
> When the balancer has no available inner services, it enters "failfast" and
...
`DEFAULT_OUTBOUND_HTTP_FAILFAST_TIMEOUT` constant in the proxy. After 3 s
with zero endpoints, the queue enters failfast and emits a `FailFastError`
for every subsequent request; the rescue handler turns it into a synthetic
`504` via `gateway_timeout`.
...
The proxy notices the new endpoint within ~1 s and exits failfast on the next
request.
